...
def apply_sigmoid_transform(image_number, input_dir, output_dir):
    ...
    pixel by pixel after normalizing the pixel values, saves the result in output_dir,
    overlays 100 random (x,y) points, and saves the overlaid image in the same folder.
    ...
    # Normalize to [-1, 1] and apply the sigmoid transform.
    ...
    # Generate 100 random (x, y) coordinates spanning the entire image.
    xs = np.random.randint(0, width, size=100)
    ys = np.random.randint(0, height, size=100)
    
    # Create a new figure and overlay the random points.
    ...
    # Save the overlaid image in the same transformed images folder.
    ...
    print(f"Overlaid image saved: {overlaid_image_path}")
    
    # Close the figure to avoid popping up the window.
    plt.close()
