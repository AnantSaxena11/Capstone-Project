...
def convert_to_greyscale(image_number, output_dir):
    ...
    grey_img = img.convert("L")  # "L" mode stands for luminance
...
# Step 3: Mark Boundaries in Blue (for the extracted gray markings)
...
# Step 4: Extract Red Pixel Positions (red boundary from IMG(i).png)
...
# New Step: Mark Boundaries on the Transformed Image
def mark_boundaries_transformed(image_number, transformed_image_path, output_dir):
    """
    Apply a boundary detection on the transformed image using the same logic.
    Saves the image with marked boundaries and a text file with boundary coordinates.
    """
    output_image_path = os.path.join(output_dir, f"transformed_boundary_image_{image_number}.png")
    output_text_path = os.path.join(output_dir, f"transformed_boundary_pixels_{image_number}.txt")
    # Open the transformed image and convert to RGB so we can mark boundaries in color.
    image = Image.open(transformed_image_path).convert('RGB')
    width, height = image.size
    new_image = image.copy()
    boundary_pixels = set()
    for y in range(height):
        for x in range(width):
            ...
            if y < height - 1:
                neighbors.append(image.getpixel((x, y + 1)))
            # For the transformed image, we consider a pixel to be "background" if it is near white.
            if any(is_white(neighbor) != is_white(current_pixel) for neighbor in neighbors):
                new_image.putpixel((x, y), (0, 0, 255))  # mark in blue
                boundary_pixels.add((x, y))
    new_image.save(output_image_path)
...
# Step 5: Compare Boundary Pixels Between Transformed and Red-Boundary Images
...
def compare_boundaries(transformed_bound_file, red_bound_file):
    """
    Compare the boundary pixels of the transformed image with the red boundary pixels.
    We compute an overlap percentage defined as:
       (number of common boundary pixels) / (total unique boundary pixels) * 100
    """
    transformed_coords = read_coordinates(transformed_bound_file)
    red_coords = read_coordinates(red_bound_file)
    if not transformed_coords and not red_coords:
        return 100.0  # Both empty: perfect match
    intersection = transformed_coords.intersection(red_coords)
    union = transformed_coords.union(red_coords)
    overlap_percentage = (len(intersection) / len(union)) * 100 if union else 0
    return overlap_percentage

# Step 6: Calculate Error Percentage (existing function for optimization analysis)
...
# Step 7: Compute and Plot Error Analysis (existing function)
...
###############################
# (The optimization functions remain unchanged)
def rastrigin(x):
    A = 10
...
def main():
    ...
    # Apply sigmoid transform and then process the transformed image boundaries.
    ...
    for i in range(1, num_images + 1):
        ...
        # Now, mark the boundaries on the transformed image.
        transformed_bound_file = mark_boundaries_transformed(i, transformed_image_path, transformed_output_dir)
        print(f"Transformed boundary for IMG{i} saved.")
        # For comparison, use the red pixel positions (from the original red-boundary image)
        red_bound_file = os.path.join(input_dir, "red_pixel_positions.txt")
        overlap = compare_boundaries(transformed_bound_file, red_bound_file)
        print(f"Boundary Overlap for IMG{i}: {overlap:.2f}%")
