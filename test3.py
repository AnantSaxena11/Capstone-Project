...
# --- New Step: Extract the Main Cell Region ---
def extract_cell_region(image_number, output_dir):
    """
    Reads the boundary pixel coordinates from boundary_pixels.txt,
    calculates the bounding box, and crops the original greyscale image
    to extract the main cell region.
    """
    boundary_file = os.path.join(output_dir, "boundary_pixels.txt")
    coords = []
        ...
        for line in f:
            ...
            if line:
                ...
                if len(parts) == 2:
                    ...
                    coords.append((x, y))
    if not coords:
        print("No boundary pixels found; cannot extract cell region.")
        return None
    xs = [p[0] for p in coords]
    ys = [p[1] for p in coords]
    min_x, max_x = min(xs), max(xs)
    min_y, max_y = min(ys), max(ys)
    # Open the original greyscale image and crop it
    greyscale_path = os.path.join(output_dir, "greyscale_image.png")
    img = Image.open(greyscale_path)
    cell_img = img.crop((min_x, min_y, max_x, max_y))
    cell_output_path = os.path.join(output_dir, "cell_extracted.png")
    cell_img.save(cell_output_path)
    print(f"Extracted cell image saved: {cell_output_path}")
    return cell_output_path
...
def main():
    ...
    for i in range(1, num_images + 1):
        ...
        # NEW STEP: Extract the main cell region using the detected boundaries
        extract_cell_region(i, output_dir)
